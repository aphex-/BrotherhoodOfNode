...
public abstract class AbstractVector3Node : Node, IPositionSampler
{
	...
	public static List<Vector3> GetInputVector3DList(Socket socket, float x, float y, float z,
													float width, float height,float depth, float seed)
	{
		...
		return node.GetVector3DList(x, y, z, width, height, depth, seed);
	}

	public abstract List<Vector3> GetVector3DList(float x, float y, float z, float width, float height, float depth, float seed);
